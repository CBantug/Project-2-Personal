...
public class MediaMaker implements Comparable<MediaMaker>, Serializable {
    ...
    public MediaMaker(String readLine, int type, BufferedReader br) throws IOException{
		...
		else if (type == 4) { producingCredits.add(media);}
		else if (type == 5){ directingCredits.add(media);}
    ...
    public static Media parseLine(String readLine, ArrayList<String> components)
    {
    	...
    	{	
    		...
    		String title = "";
    		if (Pattern.compile("([A-Za-z]+),\\s*([A-Za-z]+)\\s").matcher(readLine).find()) //if the line has a name
    		{
    			boolean containsDisambig = false;
    			for (int i = 0; i < 3; i++)
    			{
    				if (components.get(i).matches("\\([\\w]{1,2}\\)")) //if line has a disambiguation number
    				{
    	...
    	{    		
    		...
    		//episode.getSeries().setTitle(title);
    		//episode.getSeries().setYear(releaseYear);
